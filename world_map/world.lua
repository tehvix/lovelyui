    ...
    WorldMapFrame:HookScript('OnUpdate', function(self, e)
	        ...
	        if groupSize > 0 then
		        ...
		        for i = 1, groupSize do
			        ...
			        if frame then
					    ...
					    if WorldMapFrame_InWindowedMode() then
					        frame.overlay:SetPoint('TOPLEFT', frame.icon, -padding - 6, padding + 6)
					        frame.overlay:SetPoint('BOTTOMRIGHT', frame.icon, padding + 6, -padding - 6)
					    else
					    ...
					    frame.overlay:SetTexture(groupType == 'Raid' and ('Interface\\AddOns\\'..addonName..'\\world_map\\blips\\raid'..subgroup) or 'Interface\\AddOns\\'..addonName..'\\world_map\\blips\\party')
        ...
        -- AREA TEXT
	WorldMapFrameAreaLabel:SetFont(STANDARD_TEXT_FONT, 34, 'OUTLINE')
    WorldMapFrameAreaLabel:SetShadowOffset(0, -0)
    WorldMapFrameAreaDescription:SetFont(STANDARD_TEXT_FONT, 30, 'OUTLINE')
    WorldMapFrameAreaDescription:SetShadowOffset(0, -0)
    WorldMapFrameAreaPetLevels:SetFont(STANDARD_TEXT_FONT, 30, 'OUTLINE')
    WorldMapFrameAreaPetLevels:SetShadowOffset(0, -0)
